...
# `lilterm` CLI (ls / attach over SSH) ships in the bundle; --install links it into ~/.local/bin.
CLI="$(swift build -c "$CONFIG" --show-bin-path)/lilterm"
[ -f "$CLI" ] && cp "$CLI" "$APP/Contents/MacOS/lilterm"
...
if [[ "$MODE" == "--install" ]]; then
    ...
    rm -rf "$DEST"
    cp -R "$APP" "$DEST"
    # `lilterm` on the PATH (for SSH): a link into the installed bundle, so it updates with it.
    mkdir -p "$HOME/.local/bin"
    ln -sf "$DEST/Contents/MacOS/lilterm" "$HOME/.local/bin/lilterm"
